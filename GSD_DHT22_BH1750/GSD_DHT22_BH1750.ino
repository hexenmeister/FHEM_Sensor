...
void setup() {
 ...
 tinytx.msg_counter = 0;
  ...
  if(!lightMeter.begin()){
    // Error  
    tinytx.brightness = 2147483648; // Error-Marker
  }
...
void loop() {
  ...
  // Sensor requires minimum 2s warm-up after power-on.
  ...
  if(lux>=0) {
    long luxl = (long)(lux*100);
    tinytx.brightness = luxl;
  } else {
    // Error  
    tinytx.brightness = 2147483648; // Error-Marker
  }
  //if(!lightMeter.isPresent()) {
    //tinytx.brightness = 2147483648; // Error-Marker
    //lux=1;
  //}
  ...
  // Send data
  if(errorCode == DHT_ERROR_NONE || lux>=0) {
    tinytx.msg_counter++;
