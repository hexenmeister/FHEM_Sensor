...
//
// 2014-02-01 - AS - Silent mode als Default
// 2014-02-01 - AS - GSD-Marker
// 2014-02-13 - AS - Option uebernommen: "ackWithPayload" ACK mit einem Byte als "Anhang"
...
static char cmd;
static byte value, stack[RF12_MAXDATA+4], top, sendLen, dest, quiet, ackWithPayload;
...
void setup() {
  ...
  quiet = 1;
  // Introducing switch to send ACKs back 1-byte Payload
  ackWithPayload = 1;
  ...
  if (rf12_recvDone()) {
    ...
    if (rf12_crc == 0) {
      ...
      if (RF12_WANTS_ACK && (config.nodeId & COLLECT) == 0) {
        if (ackWithPayload && rf12_len > 1) {
          // ACK reply with first byte
          byte ackReply = rf12_data[1];
          if (!quiet) {
            Serial.print(" -> ack ");
            Serial.println(ackReply);
          }
          delay(random(10));
          rf12_sendStart(RF12_ACK_REPLY, &ackReply, 1);
        } else {
          // send empty ACK
          if (!quiet) Serial.println(" -> ack");
          delay(random(10));
